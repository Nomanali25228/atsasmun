...
async function run() {
    ...
    try {
        ...
        for (const targetEmail of targetEmails) {
            ...
            // Find latest registration in registrations_istanbul
            ...
            // Update database to mark acceptance letter as sent
            ...
            // Also update notifications collection if present
            await db.collection('notifications').updateMany(
                { Email: { $regex: new RegExp(`^${targetEmail}$`, 'i') } },
                { $set: { secondEmailSent: true, secondEmailSentAt: new Date().toISOString() } }
            );

            console.log(`Database records updated for: ${targetEmail}`);
            await sleep(1500);
        }
